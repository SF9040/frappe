	...
	set_body() {
		...
		frappe.model.with_doc("Number Card", this.number_card_name || this.name).then((card) => {
			if (!card) {
				...
				this.widget.prepend('<div class="widget-gradient" style="position: relative;"><div class="inner-gradient-spatial" style="width: 200px; height: 100px; border-radius: 50%; background: radial-gradient(circle at center, #000000 0%, #000000 100%); opacity: 0.25; filter: blur(57px); position: absolute; top: 100px; right: 0;"></div></div>');
