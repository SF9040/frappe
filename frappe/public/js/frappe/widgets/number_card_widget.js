	...
	set_body() {
		...
		frappe.model.with_doc("Number Card", this.number_card_name || this.name).then((card) => {
			if (!card) {
				...
				this.widget.prepend('<div style="position: relative;"><div style="width: 200px; height: 100px; border-radius: 50%; background: radial-gradient(circle at center, #0066cc 0%, #0066cc 100%); opacity: 0.25; filter: blur(57px); position: absolute; top: 100px; right: 0;"></div></div>');
				...
				this.widget.prepend(`<div style="position: relative;"><div style="width: 200px; height: 100px; border-radius: 50%; background: radial-gradient(circle at center, ${card && card.color ? card.color : '#0066cc'} 0%, ${card && card.color ? card.color : '#0066cc'} 100%); opacity: 0.25; filter: blur(57px); position: absolute; top: 100px; right: 0;"></div></div>`);
