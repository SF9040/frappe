...
export default class GridRow {
	...
	render_row(refresh) {
		...
		if (!this.row_index && !this.show_search) {
			...
			this.row_check = $(
				`<div class="row-check align-items-center justify-center d-flex sortable-handle col" style="height: auto;">
					${this.row_check_html}
				</div>`
			).appendTo(this.row);

			this.row_index = $(
				`<div class="row-index align-items-center d-flex sortable-handle col" style="height: auto;">
					<span>${txt}</span>
				</div>`
			)
				.appendTo(this.row)
	...
	add_column_configure_button() {
		...
		if (this.configure_columns && this.frm) {
			...
			this.configure_columns_button = $(`
				<div class="col grid-static-col d-flex justify-content-center" style="cursor: pointer;padding-bottom: 0;padding-top: 0;display: flex;align-items: center;">
